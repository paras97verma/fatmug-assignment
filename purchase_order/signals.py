...
def quality_rating_average(sender, instance, created, **kwargs):
    if instance.status == "Completed" and instance.quality_rating is not None:
        ...
        vendor.quality_rating_avg = (
            completed_pos.aggregate(avg_rating=Avg("quality_rating"))["avg_rating"]
            or vendor.quality_rating_avg
        )
    ...
    if instance.status in ["Completed", "Cancelled"]:
        ...
        try:
            if vendor_pos.exists():
                success_pos = vendor_pos.filter(
                    status="Completed", completion_date__lte=instance.delivery_date
                )
...
def add_performance_metrics(sender, instance, created, **kwargs):
    if not created:
        try:
            ...
            logging.info("unable to add data to Historical performance... %s", e)
